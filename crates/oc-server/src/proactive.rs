...
///
/// **两类任务的收尾时机刻意相反**，各有其理由（反直觉，但都是防御设计）：
///
/// - **一次性**：先删后跑，`DELETE` 的 rows-affected 天然是原子认领。它有两条触发
///   路径（精确 timer + 心跳兜底），必须防重，否则用户被同一件事提醒两遍。
/// - **重复**：先跑后重排（不是先占位再跑）。这类只有心跳一条路径在动（`Heartbeat`
///   串行 await 每个 tick，不会自我并发），本就无竞争；而若也改成「先占位再跑」，
///   进程在这中间被杀就会把 `next_at` 永久留空——一条每日提醒**从此再不触发**，比
///   重复提醒一次糟得多。保持后置：崩了 `next_at` 仍是旧值（已过期），下次扫描重试。
